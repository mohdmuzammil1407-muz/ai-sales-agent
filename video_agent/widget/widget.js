(function () {
  ...
  function getCurrentStepQuestion() {
    ...
    if (step === "EMAIL_FOR_CONFIRM") return "what email should we send confirmation to?";
    return "please continue with the current scheduling step.";
  }

  async function sendMeetingConfirmation(email) {
    var flow = state.meetingFlow || {};
    hideTypingIndicator();
    showTypingIndicator();
    var calendarStatus = "pending";
    var emailStatus = "pending";
    var calendarData = null;
    var calendarSuccess = false;
    try {
      var response = await fetch(getAuthApiBase() + "/api/v1/schedule-meeting", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          name: flow.userName || state.profile.name || "there",
          email: email,
          purpose: flow.meetingPurpose || "",
          time: flow.preferredTime || "",
          scheduledAt: new Date().toISOString()
        })
      });
      if (response.ok) {
        var apiData = await response.json();
        console.log("[Vidio] Schedule API response:", apiData);
        calendarData = apiData;
        calendarSuccess = Boolean(apiData && apiData.calendarCreated);
        calendarStatus =
          apiData && (apiData.calendarCreated || (apiData.calendar && apiData.calendar.success))
            ? "confirmed"
            : "pending";
        emailStatus =
          apiData &&
            apiData.email &&
            (apiData.email.lead_delivery === "email" || apiData.email.lead_delivery === "mock")
            ? "sent"
            : "pending";
      } else {
        console.warn("[Vidio] Schedule API non-OK status:", response.status);
        calendarStatus = "pending";
        emailStatus = "pending";
      }
    } catch (err) {
      console.warn("[Vidio] Meeting API post failed:", err);
      calendarStatus = "pending";
      emailStatus = "pending";
    }
    hideTypingIndicator();
    ...
    var meetLinkLine =
      calendarData && calendarData.meetLink ? "\n🔗 Google Meet: " + calendarData.meetLink : "";
    var calendarLine = calendarSuccess
      ? "\n📅 A calendar invite has been sent to " + email + "."
      : "\n📧 Our team will reach out to " + email + " to confirm the time.";

    addMessage(
      "assistant",
      "You're all set, " +
      (flow.userName || state.profile.name || "there") +
      "! ✅\n\n" +
      ...
      "\n" +
      "• Topic: " +
      (flow.meetingPurpose || "General discussion") +
      "\n" +
      "• Preferred time: " +
      (flow.preferredTime || "To be confirmed") +
      "\n" +
      "• Calendar: " + calendarStatus + "\n" +
      "• Confirmation email: " + emailStatus +
      calendarLine +
      meetLinkLine +
      "\n\n" +
      "The Ilmora Studios team will reach out shortly. 🎬",
      null,
      false
    );
    state.activeFlow = null;
  ...
  async function fetchEmailAndConfirm() {
    var flow = state.meetingFlow;
    var authUser = typeof VidioAuth !== "undefined" && VidioAuth.getUser ? VidioAuth.getUser() : null;
    ...
    if (!confirmationEmail) {
      if (flow) {
        flow.step = "EMAIL_FOR_CONFIRM";
      }
      hideTypingIndicator();
      addMessage(
        "assistant",
        "One last thing — what email should we send your meeting confirmation to?",
        null,
        false
      );
      return;
    ...
    if (flow) {
      flow.userEmail = confirmationEmail;
    }
    await sendMeetingConfirmation(confirmationEmail);
  }
  ...
  async function routeToActiveFlow(input) {
    ...
    switch (flow.step) {
      ...
      case "PURPOSE":
        ...
        // Proceed to fetch email and send to backend
        flow.step = "FETCH_EMAIL";
        showTypingIndicator();
        await fetchEmailAndConfirm();
        return true;

      case "EMAIL_FOR_CONFIRM":
        if (!isValidEmail(trimmed)) {
          addMessage("assistant", "That doesn't look right. Please enter a valid email address.", null, false);
          return true;
        }
        flow.userEmail = trimmed.toLowerCase();
        state.profile.email = flow.userEmail;
        persistProfile();
        await sendMeetingConfirmation(flow.userEmail);
        return true;

      case "FETCH_EMAIL":
        // Confirmation is already in progress; avoid noisy fallback prompts.
        if (!state.isTyping) {
          showTypingIndicator();
        }
        return true;
  ...
  function sendMessage(text, skipUserRender) {
  ...
  async function handleSend(text) {
    ...
    if (state.activeFlow === "meeting_local") {
      if (state.isSending) {
        return;
      }
      state.isSending = true;
